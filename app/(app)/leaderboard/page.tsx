      ...
      {data && entries.length > 0 && (
        <>
          ...
          <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4" aria-label="Leaderboard summary">
            <MetricCard icon="👥" label="Total readers" value={stats.readers.toLocaleString()} detail="Readers on the board" />
            <MetricCard icon="⭐" label="Weekly points" value={stats.points.toLocaleString()} detail="Points earned together" />
            <MetricCard icon="🔥" label="Longest streak" value={stats.streak.toLocaleString()} detail="Days of reading momentum" />
            <MetricCard icon="🏆" label="Top score" value={stats.topScore.toLocaleString()} detail="Best score this week" />
          </section>
...
function LeaderboardList({ entries, myChildIds }: { entries: LeaderboardEntry[]; myChildIds: number[] }) {
  return <section className="card p-0" aria-labelledby="all-readers-heading"><div className="border-b border-border/60 p-5"><p className="text-xs font-semibold uppercase tracking-[.16em] text-brand-400">Keep climbing</p><h2 id="all-readers-heading" className="mt-1 text-xl font-bold text-brand-900">All readers</h2></div>
    ...
    <div className="space-y-3 p-3 md:hidden">{entries.map((entry) => <div key={entry.id} className={`rounded-2xl border border-border/60 p-4 ${myChildIds.includes(entry.child_id) ? 'bg-brand-400/10' : 'bg-bg/30'}`}><div className="flex items-center gap-3"><span className="w-8 font-bold text-brand-400">#{entry.rank}</span><Avatar entry={entry} /><div className="min-w-0 flex-1"><p className="truncate font-bold text-brand-900">{entry.child_name?.trim() || 'Reader'}</p>{myChildIds.includes(entry.child_id) && <span className="chip mt-1">Yours</span>}</div><AchievementBadge entry={entry} /></div><div className="mt-4 flex justify-between text-sm"><span className="font-bold text-brand-600">{(entry.points || 0).toLocaleString()} pts</span><span className="text-muted" aria-label={`${entry.streak_count || 0} day streak`}>🔥 {entry.streak_count || 0} days</span></div></div>)}</div>
  </section>;
}

function LeaderboardRow({ entry, mine }: { entry: LeaderboardEntry; mine: boolean }) {
  return <tr className={`transition-colors hover:bg-brand-400/5 ${mine ? 'bg-brand-400/10' : ''}`}><td className="px-5 py-4 font-bold text-brand-400">#{entry.rank}</td><td className="px-5 py-4"><div className="flex items-center gap-3"><Avatar entry={entry} /><div><p className="font-bold text-brand-900">{entry.child_name?.trim() || 'Reader'}</p>{mine && <span className="chip mt-1">Yours</span>}</div></div></td><td className="px-5 py-4 font-bold text-brand-600">{(entry.points || 0).toLocaleString()}</td><td className="px-5 py-4 text-muted" aria-label={`${entry.streak_count || 0} day streak`}>🔥 {entry.streak_count || 0}</td><td className="px-5 py-4"><AchievementBadge entry={entry} /></td></tr>;
}
